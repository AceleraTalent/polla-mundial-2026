...
// FIFA World Cup 2026 venues per group [venue_matchday_odd, venue_matchday_even]
...
function MatchRow({ match, isFirst }: { match: MatchVM; isFirst: boolean }) {
  ...
  const venue = getVenue(match.group, match.matchday, isFirst);
...
export function FixtureView({ matches }: { matches: MatchVM[] }) {
  ...
  const tabs: { md: 1 | 2 | 3; label: string }[] = [
    { md: 1, label: "Jornada 1" },
    { md: 2, label: "Jornada 2" },
    { md: 3, label: "Jornada 3" },
  ];
