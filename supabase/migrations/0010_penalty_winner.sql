-- =====================================================================
-- Polla Mundial 2026 — Ganador de tanda de penales (bono +1 en eliminatoria)
-- =====================================================================
-- Además del marcador, en partidos de eliminatoria los usuarios pueden
-- elegir qué equipo creen que gana la tanda de penales. Si el partido
-- efectivamente se define por penales y acertaron, suman 1 punto extra.
-- Si el partido no llega a penales, o si eligieron mal, no suma ni resta.
-- =====================================================================

alter table public.predictions
  add column if not exists penalty_winner_team_id int references public.teams(id);
    ...
    select
      ...
      sum(
        ...
        *
        ...
        +
        (case
          when r.penalty_winner_team_id is not null
           and p.penalty_winner_team_id = r.penalty_winner_team_id
          then 1 else 0
        end)
      )::int as pts
